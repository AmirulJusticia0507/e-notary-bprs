...
func (r *UserRepository) Create(ctx context.Context, user *domain.User) error {
	const q = `INSERT INTO users (full_name, email, password_hash, role, is_active, photo_url, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`
	return r.db.QueryRowContext(ctx, q, user.FullName, user.Email, user.PasswordHash, user.Role, user.IsActive, user.PhotoURL, time.Now(), time.Now()).Scan(&user.ID)
}

func (r *UserRepository) FindByEmail(ctx context.Context, email string) (*domain.User, error) {
	const q = `SELECT id, full_name, email, password_hash, role, is_active, photo_url, failed_login_attempts, locked_until, created_at, updated_at
		FROM users WHERE email = $1 AND deleted_at IS NULL`
	user := &domain.User{}
	err := r.db.QueryRowContext(ctx, q, email).Scan(&user.ID, &user.FullName, &user.Email, &user.PasswordHash, &user.Role, &user.IsActive, &user.PhotoURL, &user.FailedLoginAttempts, &user.LockedUntil, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
...
func (r *UserRepository) FindByID(ctx context.Context, id int64) (*domain.User, error) {
	...
	user := &domain.User{}
	err := r.db.QueryRowContext(ctx, q, id).Scan(&user.ID, &user.FullName, &user.Email, &user.PasswordHash, &user.Role, &user.IsActive, &user.PhotoURL, &user.FailedLoginAttempts, &user.LockedUntil, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("user not found: %w", repository.ErrNotFound)
		}
...
func (r *UserRepository) List(ctx context.Context) ([]domain.User, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.ID, &user.FullName, &user.Email, &user.PasswordHash, &user.Role, &user.IsActive, &user.PhotoURL, &user.FailedLoginAttempts, &user.LockedUntil, &user.CreatedAt, &user.UpdatedAt); err != nil {
			return nil, err
		}
...
// UpdatePhoto mengganti foto profil (data-URL base64 hasil downscale frontend).
func (r *UserRepository) UpdatePhoto(ctx context.Context, id int64, photoURL string) error {
	const q = `UPDATE users SET photo_url = $1, updated_at = $2 WHERE id = $3`
	result, err := r.db.ExecContext(ctx, q, photoURL, time.Now(), id)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
